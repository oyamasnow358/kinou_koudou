...
template_csv = """日付,行動,直前の出来事（前兆）,直後の出来事（結果）,行動の機能
...
st.title("FBA（機能的行動評価）データのアップロード")
st.markdown("### CSVテンプレートのダウンロード")
st.write("""
以下のボタンをクリックすると、FBA分析で利用できる日本語版のCSVファイルのひな形をダウンロードできます。  
このテンプレートを基に、ご自身のデータを入力してください。  
ファイルはUTF-8エンコーディングで作成されており、日本語環境のExcelでも文字化けせずに扱えます。
""")
st.download_button(
